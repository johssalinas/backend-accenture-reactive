...
public class ProductoHandler {
    ...
    public Mono<ServerResponse> save(@NonNull ServerRequest request) {
        return request.bodyToMono(Producto.class)
                ...
                .flatMap(producto -> useCase.save(
                        request.headers().firstHeader(CLIENT_ID_HEADER),
                        request.headers().firstHeader(IDEMPOTENCY_KEY_HEADER),
                        producto))
                .flatMap(productoCreado -> ServerResponse
                        .created(URI.create("/api/productos/" + productoCreado.getId()))
                        .bodyValue(productoCreado))
    ...
    public Mono<ServerResponse> findById(@NonNull ServerRequest request) {
        return getPathId(request, "id")
                .flatMap(useCase::findById)
                .flatMap(productoEncontrado -> ServerResponse.ok().bodyValue(productoEncontrado))
                .onErrorResume(BusinessException.class, ApiErrorResponseMapper::mapBusiness)
                .onErrorResume(TechnicalException.class, ApiErrorResponseMapper::mapTechnical)
                .onErrorResume(error -> ApiErrorResponseMapper.mapUnknown());
    }
    ...
    public Mono<ServerResponse> findAll(ServerRequest request) {
        return ServerResponse.ok().body(useCase.findAll(), Producto.class)
                .onErrorResume(TechnicalException.class, ApiErrorResponseMapper::mapTechnical)
                .onErrorResume(error -> ApiErrorResponseMapper.mapUnknown());
    }
    ...
    public Mono<ServerResponse> updateName(@NonNull ServerRequest request) {
        return getPathId(request, "id")
                .flatMap(id -> request.bodyToMono(Producto.class)
                        .switchIfEmpty(getBusinessError(BusinessErrorMessage.INVALID_REQUEST_BODY))
    ...
    public Mono<ServerResponse> findMaxStockBySucursalForFranquicia(@NonNull ServerRequest request) {
        return getPathId(request, "franquiciaId")
                .flatMap(franquiciaId -> ServerResponse.ok()
                        .body(useCase.findMaxStockBySucursalForFranquicia(franquiciaId),
                                ProductoMaxStockPorSucursal.class))
                .onErrorResume(BusinessException.class, ApiErrorResponseMapper::mapBusiness)
                .onErrorResume(TechnicalException.class, ApiErrorResponseMapper::mapTechnical)
                .onErrorResume(error -> ApiErrorResponseMapper.mapUnknown());
    }

    private Mono<UUID> getPathId(ServerRequest request, String pathVariable) {
        return Mono.fromCallable(() -> UUID.fromString(request.pathVariable(pathVariable)))
